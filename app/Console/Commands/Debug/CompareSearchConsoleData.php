...
class CompareSearchConsoleData extends Command
{
    ...
    public function handle()
    {
        ...
        // Initialize service
        $service = new GoogleSearchConsoleService($project);
        ...
        try {
            ...
            $analytics = $service->getSearchAnalytics(['query'], $startDate, $endDate, 25);

            if ($analytics->isEmpty()) {
                $this->warn('No data returned from API.');
            } else {
                $this->info("Found {$analytics->count()} results");
                $this->newLine();
                ...
                $this->table(
                    ...
                    $analytics->take(10)->map(function ($row) {
                        return [
            ...
            if ($keyword = $this->option('keyword')) {
                $this->newLine();
                $this->info("Checking specific keyword: '{$keyword}'");

                $keywordData = $analytics->firstWhere('keys.0', $keyword);

                if ($keywordData) {
                    $this->info("Found data for keyword '{$keyword}':");
                    $this->table(
                        ['Metric', 'Value'],
                        [
                    ...
                    $this->warn("No data found for keyword '{$keyword}' in this date range.");
                }
            }
            ...
            $this->line("4. Set date range: {$startDate->format('Y-m-d')} to {$endDate->format('Y-m-d')}");
