...
class PageSpeedOverview extends StatsOverviewWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $recentResults = PageSpeedResult::where('project_id', $tenant->id)
            ->where('analyzed_at', '>=', now()->subDays(30))
            ->get();
            
        $latestMobile = PageSpeedResult::where('project_id', $tenant->id)
            ->where('strategy', 'mobile')
            ->latest('analyzed_at')
            ->first();
            
        $latestDesktop = PageSpeedResult::where('project_id', $tenant->id)
        ...
        return [
            ...
            Stat::make('30-Day Avg Performance', $avgPerformance ? round($avgPerformance, 1) . '/100' : 'No data')
                ->description("From {$totalAnalyses} analyses")
    ...
    private function getPerformanceTrend(string $strategy): array
    {
        ...
        return PageSpeedResult::where('project_id', $tenant->id)
