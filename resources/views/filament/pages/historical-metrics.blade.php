        ...
        @if (empty($this->selectedKeyword))
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <div class="text-center">
                    ...
                    <h3 class="mt-2 text-sm font-semibold text-gray-900 dark:text-gray-100">Nincs kulcsszó kiválasztva
                    </h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Válassz egy kulcsszót a "Szűrés" gomb
                        segítségével a történeti adatok megtekintéséhez.</p>
                </div>
            </div>
        @else
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                            {{ $this->selectedKeyword['keyword'] }}</h3>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Projekt:
                            {{ $this->selectedKeyword['project']['name'] ?? 'Nincs projekt' }}</p>
                        @if ($this->selectedKeyword['historical_metrics_updated_at'])
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                Utolsó frissítés:
                                {{ \Carbon\Carbon::parse($this->selectedKeyword['historical_metrics_updated_at'])->format('Y-m-d H:i') }}
                            </p>
                        @endif
                    </div>
                    <div class="text-right">
                        @if ($this->selectedKeyword['search_volume'])
                            <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">
                                {{ number_format($this->selectedKeyword['search_volume']) }}</div>
            ...
            @if (!empty($this->chartData))
                ...
                <div
                    ...
                    <h4 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Keresési volumen trendje
                    </h4>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach ($this->chartData as $index => $row)
                                    @php
                                        $previousValue =
                                            $index > 0 ? $this->chartData[$index - 1]['search_volume'] : null;
                                        $change = $previousValue
                                            ? (($row['search_volume'] - $previousValue) / $previousValue) * 100
                                            : null;
                                    @endphp
                                    <tr>
                                        <td
            ...
            @else
                <div
                    ...
                    <div class="text-center">
                        <div
                            class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-yellow-100 dark:bg-yellow-900">
                            <x-heroicon-o-exclamation-triangle class="h-6 w-6 text-yellow-600 dark:text-yellow-400" />
                        </div>
                        <h3 class="mt-2 text-sm font-semibold text-gray-900 dark:text-gray-100">Nincs történeti adat
                        </h3>
