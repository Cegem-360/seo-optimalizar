    ...
    <div class="space-y-6">
        ...
        <div class="border-b border-gray-200 dark:border-gray-700">
            <nav class="-mb-px flex space-x-8">
                <button
                    wire:click="$set('selectedService', 'search_console')"
                    @class([
                        'whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm',
                        'border-primary-500 text-primary-600 dark:text-primary-400' => $selectedService === 'search_console',
                        'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300' => $selectedService !== 'search_console',
                    ])
                >
                    Search Console
                </button>
                <button
                    wire:click="$set('selectedService', 'analytics')"
                    @class([
                        'whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm',
                        'border-primary-500 text-primary-600 dark:text-primary-400' => $selectedService === 'analytics',
                        'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300' => $selectedService !== 'analytics',
                    ])
                >
                    Analytics
                </button>
                <button
                    wire:click="$set('selectedService', 'google_ads')"
                    @class([
                        'whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm',
                        'border-primary-500 text-primary-600 dark:text-primary-400' => $selectedService === 'google_ads',
                        'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300' => $selectedService !== 'google_ads',
                    ])
                >
                    Google Ads
                </button>
        ...
        @if($selectedService === 'search_console' && $searchConsoleData)
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                    ...
                    <dl class="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-4">
                        @foreach($searchConsoleData['aggregated_metrics'] as $key => $value)
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ ucwords(str_replace('_', ' ', $key)) }}</dt>
                                <dd class="mt-1 text-2xl font-semibold text-gray-900 dark:text-gray-200">{{ $value }}</dd>
                            </div>
                        @endforeach
                    </dl>
                </div>
                ...
                {{-- Raw Data Table --}}
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Raw Data ({{ count($searchConsoleData['rows']) }} rows)</h3>
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Query</th>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Impressions</th>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach($searchConsoleData['rows'] as $row)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">{{ Str::limit($row['query'], 30) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">{{ Str::limit($row['page'], 40) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">{{ $row['country'] }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">{{ $row['device'] }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">{{ $row['clicks'] }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">{{ $row['impressions'] }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">{{ $row['ctr'] }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 dark:text-gray-200">{{ $row['position'] }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
        ...
        @elseif($selectedService === 'search_console')
            <div class="text-center py-12 bg-white dark:bg-gray-800 rounded-lg">
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Click "Fetch Search Console Data" to load data.</p>
            </div>
        @endif
        ...
        @if($selectedService === 'analytics' && $analyticsData)
            <div class="space-y-6">
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Analytics Data</h3>
